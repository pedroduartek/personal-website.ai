import csharpIcon from '../images/csharp_icon.webp'
...
export default function TechIcon({ tech, className }: Props) {
    ...
    return (
      ...
      >
        <svg className="w-full h-full" viewBox="0 0 24 24" aria-hidden>
          ...
          <path
            d="M4 14s3-4 8-4 8 4 8 4v2s-3-3-8-3-8 3-8 3v-2z"
            fill="#fff"
            opacity="0.95"
          />
        </svg>
  ...
  if (
    ...
    t.includes('homeassistant') ||
    t.includes('home assistant os')
  ) {
    return (
      <span
        className={`${className ?? ''} inline-block rounded-md overflow-hidden`}
  ...
  if (
    t.includes('iot') ||
    t.includes('iot automation') ||
    t.includes('iotautomation')
  ) {
    return (
      <span
        className={`${className ?? ''} inline-block rounded-md overflow-hidden`}
